...
export class VRHoverFeedbackSystem {
    ...
    _buttonFeedback(hover) {
        ...
        if (this._lastButton && this._lastButton !== button) {
            this._lastButton.mesh.scale.setScalar(1);
            this._lastButton.mesh.position.z = 0.10;
        }

        if (button) {
            button.mesh.scale.setScalar(1.16);
            button.mesh.position.z = 0.18;
        }
    ...
    _reticleFeedback(controller, hover) {
        if (!this._reticles.has(controller)) {
            ...
            ring.name = `vr:hoverCursor:${controller.userData.index ?? 0}`;

            const dot = new THREE.Mesh(
                new THREE.SphereGeometry(0.030, 24, 24),
            ...
            const halo = new THREE.Mesh(
                new THREE.RingGeometry(0.052, 0.068, 36),
                new THREE.MeshBasicMaterial({
                    color: 0xffffff,
                    transparent: true,
                    opacity: 0.92,
            ...
            halo.renderOrder = 1999;
            ring.add(halo);

            this.input.runtime.scene.add(ring);
            this._reticles.set(controller, ring);
        }
        ...
        if (hover?.point) {
            ...
            reticle.scale.setScalar(hover.kind === 'ui' ? 1.7 : 1.1);
            reticle.lookAt(this.input.runtime.camera.position);
        } else {
